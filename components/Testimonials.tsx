...
const Testimonials: React.FC = () => {
  return (
    <section id="depoimentos" className="py-32 bg-[#F1F8F1] overflow-hidden scroll-mt-20 border-y border-[#2E7D32]/5">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
          ...
          <div className="flex gap-2">
            <div className="w-16 h-2 bg-[#2E7D32] rounded-full" />
            <div className="w-8 h-2 bg-[#2E7D32]/30 rounded-full" />
            <div className="w-4 h-2 bg-[#2E7D32]/10 rounded-full" />
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {TESTIMONIALS.map((t) => (
            <div key={t.id} className="p-12 rounded-[3rem] bg-white border border-zinc-200 relative hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
              <Quote className="absolute top-10 right-10 w-16 h-16 text-[#2E7D32] opacity-10" />
              <div className="flex items-center gap-5 mb-8 relative z-10">
                <div className="p-1 rounded-full bg-[#F1F8F1] border border-[#2E7D32]/10">
